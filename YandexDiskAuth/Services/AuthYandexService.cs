...
namespace YandexDiskAuth.Services {
    public class AuthYandexService {
        ...
        internal bool RequestCodeAsync() {
            ...
            AddQueryParam("device_id", _yandexAppSettings.DeviceId, false);

            string url = sb.ToString();
            using var process = ProcessHelper.OpenUrl(url);
            return process != null;
        }

        /// <summary>
        /// Exchange received auth code on OAuth token 
        /// </summary>
        internal async Task<(bool Success, CodeToTokenModel TokenInfo, CodeToTokenErrorModel Error)> ExchangeCodeOnToken(string code) {
            ...
            if (response.IsSuccessful) {
                ...
                return (false, null, JsonSerializer.Deserialize<CodeToTokenErrorModel>(response.Content));
            }
        }
    }
}